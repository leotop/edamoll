...
function ValidateForm(form, ajax_type, ajax_post, preorder)
{
    ...
    if (form.REVIEW_AUTHOR && (form.REVIEW_AUTHOR.value.length < 2))
    {
         $("#error_name").html("Вы должны ввести имя")
    }
	if (MessageLength < 2)
        $("#error_message").html( oErrors['no_message']);
	else if ((MessageMax != 0) && (MessageLength > MessageMax))
        $("#error_message").html(oErrors['max_len'].replace(/\#MAX_LENGTH\#/gi, MessageMax).replace(/\#LENGTH\#/gi, MessageLength));
