        ...
        <div class="bx-auth">
            <form method="post" name="bx_auth_services<?=$arParams["SUFFIX"]?>" target="_top" action="<?=$arParams["AUTH_URL"]?>">
                <?if($arParams["~SHOW_TITLES"] != 'N'):?>
                    ...
                    <?endif;?>
                <?if($arParams["~FOR_SPLIT"] != 'Y'):?>
                    <div class="bx-auth-services bx-auth-serv-icons">
                        <?
                            foreach($arAuthServices as $service) {
                            ?>
                            <? $gID=""; 
                                if ($service["ID"] =="Odnoklassniki" || $service["ID"] =="VKontakte" || $service["ID"] =="Facebook") $gID=";$('#bx_auth_serv_".$arParams["SUFFIX"].$service["ID"].">a').click();"; ?>
                            <a href="javascript:void(0)" onclick="BxShowAuthService('<?=$service["ID"]?>', '<?=$arParams["SUFFIX"]?>')<?=$gID?>" 
                                id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>">
                                <i class="bx-ss-icon <?=htmlspecialcharsbx($service["ICON"])?>">
                                </i>
                                <b>
