...
function generateThreeDates(){
	$addedDays = 0;
	 if(CModule::IncludeModule("iblock"))
	 {
		...
		$arDates["2"]["timestamp"] = findNextDate($arDates["1"]["timestamp"], false);
		$arDates["3"]["timestamp"] = findNextDate($arDates["2"]["timestamp"], false);
		
		$arDates["1"]["TEXT"] = rus_date("d.m.Y (D)", $arDates["1"]["timestamp"]);
		$arDates["2"]["TEXT"] = rus_date("d.m.Y (D)", $arDates["2"]["timestamp"]);
		$arDates["3"]["TEXT"] = rus_date("d.m.Y (D)", $arDates["3"]["timestamp"]);
	}
	return $arDates;
}
					...
					elseif($arProperties["TYPE"] == "TEXT")
					{
						...
						?>
							...
							<select name="<?=$arProperties["FIELD_NAME"]?>" id="<?=$arProperties["FIELD_NAME"]?>" size="<?=$arProperties["SIZE1"]?>">
								...
								<option value="<?=$arDates["2"]["TEXT"]?>"><?=$arDates["2"]["TEXT"]?></option>
								<option value="<?=$arDates["3"]["TEXT"]?>"><?=$arDates["3"]["TEXT"]?></option>
							</select>
						<?} else {?>
