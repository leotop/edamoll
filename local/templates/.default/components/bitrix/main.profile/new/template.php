...
*/?>
	<tr>
		<td style="width:151px;"><?=GetMessage('NAME')?></td>
		<td><input  class='noedit'type="text" name="NAME" maxlength="50" value="<?=$arResult["arUser"]["NAME"]?>" readonly/></td>
		<td></td>
</tr>


<tr>
			<td><?=GetMessage("USER_BIRTHDAY_DT")?>:</td>
	<td><input  class='noedit'  type="text" name="PERSONAL_BIRTHDAY" maxlength="50" value="<?=$arResult["arUser"]["PERSONAL_BIRTHDAY"]?>" readonly/>
<?/*
...
*/?></td>
	<td></td>
		...
		<tr>
			...
			<td><input  class='noedit' type="text" name="PERSONAL_PHONE" maxlength="255" value="<?=$arResult["arUser"]["PERSONAL_PHONE"]?>" readonly/></td>
<td class="gray_t">

	Телефон, по которому мы сможем связаться <br />с вами для того, чтобы доставить вам заказ

			</td>
</tr>
	<tr>
		<td><?=GetMessage('EMAIL')?><span class="starrequired"></span></td>
		<td><input  class="noedit"  type="text" name="EMAIL" maxlength="50" value="<? echo $arResult["arUser"]["EMAIL"]?>" readonly /></td>
<td class="gray_t">

	Электронная почта, на которую будет поступать <br />информация о состоянии вашего заказа

			</td>
	</tr>

		<tr style="display:none;">
		<td>Логин<span class="starrequired">*</span></td>
...
<table class="data-table profile-table">
	...
	<?endif;?>
	<?// ******************** <p><?echo $arResult["GROUP_POLICY"]["PASSWORD_REQUIREMENTS"];</p>/User properties ***************************************************?>


	<p>
<input id="save_btn" class="personal_sbmt2" type="submit" name="save" value="СОХРАНИТЬ ИЗМЕНЕНИЯ">
<input id="change_btn" class="personal_sbmt" type="button" name="change" value="ИЗМЕНИТЬ ДАННЫЕ" onclick="$('.noedit').prop('readonly',false);$('.noedit').removeClass('noedit');$('#change_btn').css('display','none');$('#changep_btn').css('display','none');$('#save_btn').css('display','block');$('.gray_t').css('display','block');">
<?if($arResult["arUser"]["EXTERNAL_AUTH_ID"] == ''):?>
<input id="changep_btn" id="change_btn"  class="personal_sbmt" type="button" name="change_pass" value="Изменить пароль" onclick="$('.darker').css('display','block');">
<?endif?>

</p>

<div class="darker">
<div class="pass_edit_div">
	<div><span class="blacktext">ИЗМЕНЕНИЕ ПАРОЛЯ ДЛЯ ВХОДА</span></div>
<table class="profile-table data-table">
	<tr>
		<td>Новый пароль</td>
		<td><input id="pwd1" type="password" name="NEW_PASSWORD" maxlength="50" value="" autocomplete="off" class="bx-auth-input" />
...
<?endif?>
	<tr>
		<td>Повторите ввод<br />нового пароля</td>
		<td><input  id="pwd2" type="password" name="NEW_PASSWORD_CONFIRM" maxlength="50" value="" autocomplete="off" /></td>
	</tr>
</table>
	<div style="text-align:right">
	<input onchange="if ($('#pwd2').get(0).type=='password') {$('#pwd2').get(0).type='text';$('#pwd1').get(0).type='text';} else {$('#pwd2').get(0).type='password';$('#pwd1').get(0).type='password';}" name="fff" type="checkbox" value="false">Показывать введенные символы
	</div>
<div>
<input class="personal_sbmt" type="submit" name="save" value="ИЗМЕНИТЬ ПАРОЛЬ">
<input class="personal_sbmt" type="button" name="nosave" value="ОТМЕНА"  onclick="$('.darker').css('display','none');$('#pwd2').val('');$('#pwd1').val('');">
</div>
</div></div>
