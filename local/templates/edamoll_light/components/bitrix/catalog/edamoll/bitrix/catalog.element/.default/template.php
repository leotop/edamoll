...
<div class="catalog-element-image">
	...
	<div class="catalog-element-likes"></div>
...
<div class="flr catalog-element-prop">
	<b class="propname">Лот№ (ID Товара)</b>&nbsp;<?=$arResult["ID"]?><br />
				...
				<?foreach($arResult["DISPLAY_PROPERTIES"] as $pid=>$arProperty):?>
				<? if ($arProperty["CODE"]=="MARKA" || $arProperty["CODE"]=="STRANA") {?>

	<b class="propname"><?=$arProperty["NAME"]?></b>&nbsp;<?
	...
	<span class="pinktext">Описание товара</span><br />
				...
				<? if ($arProperty["CODE"]!=="SALELEADERS" && $arProperty["CODE"]!=="MARKA" && $arProperty["CODE"]!=="STRANA") {?>
...
<div class="flr catalog-element-prices">
...
<div class="pmcont">
<a class="minus" onclick="minus('q_<?=$arResult['ID']?>')"></a>
								<input id="q_<?=$arResult['ID']?>" type="text" size="6" class="quantity" type="text" name="<?echo $arParams["PRODUCT_QUANTITY_VARIABLE"]?>" value="1">
<a class="plus" onclick="plus('q_<?=$arResult['ID']?>')"></a>
						</div>
				...
				<input class="btncart" type="submit" name="<?echo $arParams["ACTION_VARIABLE"]."ADD2BASKET"?>" value="<?echo GetMessage("CATALOG_ADD_TO_BASKET")?>">
				</form>
			<?else:?>
				<noindex>
