...
<?
    ...
    if ($arResult["UF_DESC"] <> "") {
        ...
        $strdescmeta = "В категории ".$arResult["NAME"]." Интернет-магазина edamoll.ru представлен широкий ассортимент товаров, которые Вы можете приобрести по низкой цене.".
        " Покупайте товары категории ".$arResult["NAME"]." с доставкой по Москве и области.";
    }
    if ($arResult["UF_PAGETITLE"] <> "") {
        $title = $arResult["UF_PAGETITLE"];
    } else {
        $title = $arResult["NAME"] . " - купить в интернет-магазине edamoll.ru по низкой цене с доставкой по Москве и Московской области.";
    }
                        ...
                        <?
                            ...
                            <div class="catalog_item_buy">
                                <form action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
                                    <a class="minus" onclick="minus('q_<?=$arItem['ID']?>','<?=$base_quantity?>')">
                                    </a>
                                    <?
                                        if($arItem["PROPERTIES"]["CML2_BASE_UNIT"]["VALUE"]=="КГ" || $arItem["PROPERTIES"]["CML2_BASE_UNIT"]["VALUE"]=="кг") {
                                            $g_qu = "1.0";
                                        } else {
                                            $g_qu = "1";
                                        }
                                    ?>
                                    <input  onchange="edit_q('q_<?=$arItem['ID']?>','<?=$base_quantity?>')" id="q_<?=$arItem['ID']?>" type="text"
                                        size="6" class="quantity" type="text" name="<?echo $arParams["PRODUCT_QUANTITY_VARIABLE"]?>"
                                        value="<?=$g_qu?> <?=$base_quantity?>">
                                    <a class="plus" onclick="plus('q_<?=$arItem['ID']?>','<?=$base_quantity?>')">
                                    </a>
                                    ...
                                    <a href="<?=$arItem["ADD_URL"]?>" name="<?echo $arParams["ACTION_VARIABLE"]."ADD2BASKET"?>" class="addtoCart" value=""
                                        onclick="return addToCart(this, '<?=str_replace("'","&lsquo;",$arItem["NAME"])?>','q_<?=$arItem['ID']?>','<?=$base_quantity?>');">
                                    </a>
                                </form>
                            </div>
                            <?
